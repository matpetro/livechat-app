...
export default function Chat({ username, room, socket }) {
    ...
    const sendMessage = () => {
      if (message.trim() !== ""){
        socket.emit("send_message", { message, room, username })  // emit some event so that another person can listen to it
    ...
    React.useEffect(() => {
      ...
      socket.on("receive_message", (data) => setMessagesRecieved(prev => [...prev, {username: data.username, message: data.message}]))
